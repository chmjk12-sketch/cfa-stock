...
from sqlalchemy import (
    Column, String, Float, Boolean, Integer, DateTime, Date, BigInteger,
...
class MarketDataModel(Base):
    ...
    stock = relationship("StockModel", backref="market_data")


class InstitutionalForecastModel(Base):
    __tablename__ = "institutional_forecast"

    id = Column(Integer, primary_key=True)
    ticker = Column(String(20), ForeignKey("stocks.ticker"), index=True)
    report_date = Column(Date)

    eps_forecast_optimistic = Column(Float)
    eps_forecast_neutral = Column(Float)
    eps_forecast_pessimistic = Column(Float)
    net_profit_growth = Column(Float)
    revenue_growth = Column(Float)

    target_price_optimistic = Column(Float)
    target_price_neutral = Column(Float)
    target_price_pessimistic = Column(Float)

    source_count = Column(Integer)
    source_names = Column(Text)

    peg = Column(Float)
    upside_optimistic = Column(Float)
    upside_neutral = Column(Float)
    upside_pessimistic = Column(Float)

    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())

    stock = relationship("StockModel", backref="institutional_forecast")


class KlineDailyModel(Base):
